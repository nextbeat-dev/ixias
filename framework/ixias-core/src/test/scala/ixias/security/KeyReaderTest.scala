...
class KeyReaderTest extends FunSuite {
  ...
  test("KeyReader should return the correct metadata") {
    ...
    assertEquals(
      reader.getMetadata,
      "{\"encrypted\":false,\"purpose\":\"SIGN_AND_VERIFY\",\"versions\":[{\"exportable\":false,\"versionNumber\":0,\"status\":\"PRIMARY\"}],\"name\":\"Imported from HMAC\",\"type\":\"HMAC_SHA1\"}"
    )
  }
  ...
  test("AesKeyReader should return the correct key") {
    ...
    assertEquals(
      reader.getKey,
      "{\"mode\":\"CBC\",\"size\":64,\"hmacKey\":{\"size\":64,\"hmacKeyString\":\"dGVzdC1rZXk\"},\"aesKeyString\":\"dGVzdC1rZXk\"}"
    )
  }
  ...
  test("AesKeyReader should return the correct metadata") {
    ...
    assertEquals(
      reader.getMetadata,
      "{\"encrypted\":false,\"purpose\":\"DECRYPT_AND_ENCRYPT\",\"versions\":[{\"exportable\":false,\"versionNumber\":0,\"status\":\"PRIMARY\"}],\"name\":\"Imported from AES\",\"type\":\"AES\"}"
    )
  }
}
