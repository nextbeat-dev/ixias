...
trait LabelledGenericMerger[T, U] {
  ...
  object mf extends Poly2 {
    implicit def opt1[A]: Case[A, A] { type Result = A } = at[A, A]((v1, v2) => v2)
    implicit def opt2[A]: Case[Option[A], Option[A]] { type Result = Option[A] } =
      at[Option[A], Option[A]]((v1, v2) => v2 orElse v1)
  }

  /** Copies all enumerable own properties from source object to a target object.
    */
  ...
  /** Copies all enumerable own properties from source generic values to a target object.
    */
