...
/** The Enums based on sealed classes
  *
  * < Example > sealed abstract class Color(red: Double, green: Double, blue: Double) object Color extends EnumOf[Color]
  * { case object Red extends Color(1, 0, 0) case object Green extends Color(0, 1, 0) case object Blue extends Color(0,
  * 0, 1) case object White extends Color(0, 0, 0) case object Black extends Color(1, 1, 1) }
  */
...
/** The operation of EnumStatus
  */
object EnumStatus {
  abstract class Of[T <: EnumStatus](implicit ttag: ClassTag[T]) extends Enum.Of[T] {
...
/** The operation of EnumBitFlags
  */
object EnumBitFlags {
  abstract class Of[T <: EnumBitFlags](implicit ttag: ClassTag[T]) extends Enum.Of[T] {
  ...
  abstract class Of[T <: Enum](implicit ttag: ClassTag[T]) { self =>
    ...
    private lazy val fields: Option[List[Any]] = {
      ...
      if (symbol.isModuleClass) {
        Some(sortedInnerModules(symbol).map(m => mirror.reflectModule(m.asModule).instance))
      } else {
        None
      }
    }
