...
object PBKDF2 {
  ...
  /** Uses PBKDF2 and random salt generation to create a hash based on some input.
    ...
    *   (optional) The salt.
    * @param iterations
    *   (optional) The number of iterations to be used.
    * @param length
    *   (optional) The length the key will be cropped to fit.
    * @param algo
    *   (optional) The hashing algorithm.
    */
  def hash(
    input:      String,
    salt:       Array[Byte] = internals.salt,
    iterations: Int = internals.iterations,
