...
case class PolyLeftJoinSyntax[R, P <: Poly2](underlying: R, poly2: P) {
  ...
  /** Join operation
    */
  def apply[L <: HList](haystack: L)(implicit folder: LeftFolder[L, R, poly2.type]): folder.Out =
...
object PolyLeftJoinSyntax {
  ...
  trait Rule[R] extends Poly2 {
    ...
    /** Implicit convert: T => Seq[T]
      */
    implicit def caseAnyRef[T](implicit st: Case[R, Seq[T]]): Case[R, T] { type Result = st.Result } =
      at[R, T]((root, haystack) => this(root, Seq(haystack)))

    /** Implicit convert: Option[T] => Seq[T]
      */
    implicit def caseOpt[T](implicit st: Case[R, Seq[T]]): Case[R, Option[T]] { type Result = st.Result } =
