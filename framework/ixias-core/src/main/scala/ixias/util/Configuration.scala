...
case class Configuration(underlying: Config = ConfigFactory.load()) {
  ...
  /** Reads a value from the underlying implementation. If the value is not set this will return None, otherwise returns
    * Some.
    */
...
object ConfigLoader {
  ...
  implicit val duration1Loader: ConfigLoader[JavaDuration]     = ConfigLoader(_.getDuration)
  implicit val duration2Loader: ConfigLoader[Duration] = ConfigLoader(config =>
    path =>
      (config.getIsNull(path)) match {
        case true  => Duration.Inf
        case false => config.getDuration(path).toNanos.nanos
      }
  )

  // Retrieves a configuration value as a Seq of specified type.
  ...
  implicit val seqFiniteLoader: ConfigLoader[Seq[FiniteDuration]] =
    ConfigLoader(_.getDurationList).map(_.asScala.map(_.toNanos.nanos).toSeq)
  implicit val seqDuration1Loader: ConfigLoader[Seq[JavaDuration]] =
    ConfigLoader(_.getDurationList).map(_.asScala.toSeq)
  implicit val seqDuration2Loader: ConfigLoader[Seq[Duration]] =
    ConfigLoader(_.getDurationList).map(_.asScala.map(_.toNanos.nanos).toSeq)
