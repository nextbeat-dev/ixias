...
/** The writer to output a csv file.
  */
case class CsvWriter(writer: Writer)(implicit format: CsvFormat) extends Closeable with Flushable {
  ...
  def shouldQuote(field: String, quote: QuoteStyle): Boolean =
    quote match {
      ...
      case QuoteStyle.MINIMAL => {
        field.indexWhere(c => format.CSV_WRITE_MINIMAL_QUOTE_SPECS.contains(c)) != -1
      }
...
object CsvWriter {
  ...
  def apply(file: File, append: Boolean, encoding: String)(implicit format: CsvFormat): CsvWriter =
    apply(new FileOutputStream(file, append), encoding)(format)
