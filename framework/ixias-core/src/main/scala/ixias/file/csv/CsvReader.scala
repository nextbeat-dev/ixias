...
object CsvReader {
  def apply(filename: String, encoding: String = "UTF-8")(implicit format: CsvFormat = CsvDefaultFormat): CsvReader =
    ...
    source.foldLeft(0) { (pos, current) =>
      {
        ...
        if (
          !enclosed && (current.toInt == format.ASCII_LINE_FEED
    ...
    lines.foldLeft(Seq[Seq[String]]()) { (rows, line) =>
      ...
      line.foldLeft(0) { (pos, current) =>
        ...
        if (!enclosed) {
          ...
          if (
            current.toInt == format.CSV_FIELD_TERM_CHAR
