...
/** An Entity Action that can be executed on a persistence database.
  */
trait EntityIOAction[K <: @@[_, _], M <: EntityModel[K]] extends IOAction { self: Repository[K, M] =>
  ...
  // --[ Methods ]--------------------------------------------------------------
  /** Defines the default value computation for the map, returned when a identity is not found The method implemented
  ...
  /** Retrieves the value which is associated with the given identity. This method invokes the `default` method of the
    * map if there is no mapping from the given identity to a value.
    */
  ...
  /** Returns the value associated with a identity, or a default value if the identity is not contained in the
    * repository.
    */
  ...
  /** If the dataset already contains a mapping for the identity, it will be overridden by the new value.
    */
  def update(entity: EntityEmbeddedId): Future[Option[EntityEmbeddedId]]

  /** Removes a identity from this map, returning the value associated previously with that identity as an option.
    */
