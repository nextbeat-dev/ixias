...
trait SlickConfig extends BasicDatabaseConfig {
  ...
  // --[ Methods ]--------------------------------------------------------------
  /** Get the property controls the minimum number of idle connections that Driver tries to maintain in the pool,
  ...
  /** Get the property controls the maximum size that the pool is allowed to reach, including both idle and in-use
  ...
  /** Get the maximum number of milliseconds that a client will wait for a connection from the pool. If this time is
    * exceeded without a connection becoming available, a SQLException will be thrown from
    */
