...
trait JavaTimeBindable {
  ...
  /** QueryString binder for LocalDate
    */
  implicit object queryStringBindableLocalDate
    extends QueryStringBindable.Parsing[LocalDate](
  ...
  /** QueryString binder for LocalDate -> LocalDate
    */
  implicit object queryStringBindableLocalDateRange
    extends QueryStringBindable.Parsing[LocalDateRange](
      (s: String) =>
        s.split(",").map(LocalDate.parse).toSeq match {
  ...
  /** QueryString binder for YearMonth
    */
  implicit object queryStringBindableYearMonth
    extends QueryStringBindable.Parsing[YearMonth](
  ...
  /** QueryString binder for YearMonth -> YearMonth
    */
  implicit object queryStringBindableYearMonthRange
    extends QueryStringBindable.Parsing[YearMonthRange](
      (s: String) =>
        s.split(",").map(YearMonth.parse).toSeq match {
          case Seq(v1, v2) => (v1, v2)
          case _           => throw new IllegalArgumentException("The date-range value syntax is not valid")
        },
