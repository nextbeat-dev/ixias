...
trait QueryStringHelper extends binder.Box {
  ...
  implicit def toQueryStringHelperBoxCsvOps[A](
    bindable: QueryStringBindable[BoxCsv[A]]
  ): QueryStringHelper.QueryStringHelperBoxCsvOps[A] =
...
object QueryStringHelper extends binder.Box {
  ...
  case class QueryStringHelperOps[A](val bindable: QueryStringBindable[A]) extends AnyVal {
    ...
    /** Get a value from queryString parameter.
      */
    def bindOption(key: String)(implicit params: Map[String, Seq[String]]): Either[Result, Option[A]] =
      _bindOption(key).left.map(BadRequest(_))

    /** Get a value from queryString parameter.
      */
    private[mvc] def _bindOption(key: String)(implicit params: Map[String, Seq[String]]): Either[String, Option[A]] =
  ...
  case class QueryStringHelperBoxCsvOps[A](val bindable: QueryStringBindable[BoxCsv[A]]) extends AnyVal {
    ...
    /** Get a value from queryString parameter.
      */
    def bind(key: String)(implicit params: Map[String, Seq[String]]): Either[Result, Seq[A]] =
      _bind(key).left.map(BadRequest(_))

    /** Get a value from queryString parameter.
      */
    private[mvc] def _bind(key: String)(implicit params: Map[String, Seq[String]]): Either[String, Seq[A]] =
      bindable.bind(key, params) match {
        case None           => Right(Nil)
