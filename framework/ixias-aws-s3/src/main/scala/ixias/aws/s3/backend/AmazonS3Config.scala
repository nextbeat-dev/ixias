...
trait AmazonS3Config {
  ...
  protected def getAWSAccessKeyId(implicit dsn: DataSourceName): Try[String] =
    Try(readValue(_.get[Option[String]](CF_S3_ACCESS_KEY)).get)
  ...
  protected def getAWSSecretKey(implicit dsn: DataSourceName): Try[String] =
    Try(readValue(_.get[Option[String]](CF_S3_SECRET_KEY)).get)

  /** Gets a region enum corresponding to the given region name.
    */
  def getAWSRegion(implicit dsn: DataSourceName): Try[Regions] =
    Try(Regions.fromName(readValue(_.get[Option[String]](CF_S3_REGION)).get))

  /** Gets the name of the bucket where this request will upload a new object to. In order to upload the object, users
    * must have Permission.Write permission granted.
    */
  def getBucketName(implicit dsn: DataSourceName): Try[String] =
    Try(readValue(_.get[Option[String]](CF_S3_BUCKET_NAME)).get)

  /** Gets the amount of time to wait (in milliseconds) when initially establishing a connection before giving up and
    * timing out.
    */
  ...
  def readValue[A](f: Configuration => Option[A])(implicit dsn: DataSourceName): Option[A] =
    (dsn.name.toSeq.map(name => dsn.path + "." + dsn.resource + "." + name) ++ Seq(
