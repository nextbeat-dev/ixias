...
//~~~~~~~~~~~~~~~~~~~~~~~~~~~
trait AmazonS3Repository[P <: JdbcProfile] extends SlickRepository[File.Id, File, P] with SlickResource[P] {
  import api._
  ...
  def updateImageSize(fid: Id, size: File.ImageSize): Future[Option[EntityEmbeddedId]] =
    RunDBAction(FileTable) { slick =>
      for {
        ...
        _   <- slick.unique(fid).map(c => (c.width, c.height)).update((Some(size.width), Some(size.height)))
      } yield old
    }
