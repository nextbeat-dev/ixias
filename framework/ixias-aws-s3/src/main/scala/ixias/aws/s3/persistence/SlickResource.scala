...
trait SlickResource {
  ...
  protected class FileTable(tag: Tag) extends Table[File](tag, getMetaTableName) {
    ...
    // Columns
    /* @1 */ def id        = column[Option[Long]]  ("id",         UInt64, O.PrimaryKey, O.AutoInc)
    /* @2 */ def region    = column[String]        ("region",     Utf8Char32)
    /* @3 */ def bucket    = column[String]        ("bucket",     Utf8Char32)
    /* @4 */ def key       = column[String]        ("key",        Utf8Char255)
    /* @5 */ def typedef   = column[String]        ("typedef",    Utf8Char32)
    /* @6 */ def width     = column[Option[Int]]   ("width",      UInt16)
    /* @7 */ def height    = column[Option[Int]]   ("height",     UInt16)
    /* @8 */ def updatedAt = column[LocalDateTime] ("updated_at", TsCurrent)
    /* @9 */ def createdAt = column[LocalDateTime] ("created_at", Ts)
