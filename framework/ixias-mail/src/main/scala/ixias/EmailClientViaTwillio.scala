...
class EmailClientViaTwillio extends EmailClient with EmailConfig {
  ...
  /** Send an email with the provided data.
    */
  override def send(to: UserEmail, tpl: EmailTemplate[_])(implicit ctx: ExecutionContext): Future[String] =
    for {
  ...
  /** Send an email with the provided data.
    */
  override def send(to: UserEmail, from: UserEmail, tpl: EmailTemplate[_])(implicit
